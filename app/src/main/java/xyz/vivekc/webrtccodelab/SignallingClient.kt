...
internal class SignallingClient {
    ...
    private val socket = IO.socket(Constants.SOCKET_HOST_URL)
    ...
    fun init(signalingInterface: SignalingInterface?) {
        println("GIO ${SignallingClient::class.java.simpleName}")
...
//            IO.setDefaultSSLContext(sslContext)
            ...
            socket.on(Socket.EVENT_CONNECT) {args: Array<Any?>? ->
                Log.d(TAG, "connected with: args = [" + Arrays.toString(args) + "]")

                Log.d(TAG, "init() called $socket")
            ...
            //peer joined event
            socket.on("create or join") { args: Array<Any?>? ->
    ...
    fun emitMessage(message: SessionDescription?) {
        try {
            ...
            Log.d(TAG, e.message ?: "jsonexception")
            e.printStackTrace()
        }
    }
